...
```bash
python3 /Users/thierry/.agents/execution/scrape_linkedin.py \
...
```bash
python3 /Users/thierry/.agents/execution/scrape_linkedin.py --url "YOUR_URL" --check-only
```
Show the user: prospect count, confirm filters look right.

### Step 2: Scrape + Download
```bash
python3 /Users/thierry/.agents/execution/scrape_linkedin.py --url "YOUR_URL" --name "My Campaign" --limit 50 --skip-enrichment --skip-sheets
```
...
```bash
python3 /Users/thierry/.agents/execution/enrich_leads.py --input .tmp/YOUR_RAW_CSV.csv
```
...
```bash
python3 /Users/thierry/.agents/execution/export_leads_to_sheets.py --input .tmp/YOUR_ENRICHED_CSV.csv --create "LinkedIn Leads - Campaign Name" --valid-only
```
